...
export const BasicContextExample = () => {
    ...
    return <div>
        ...
        <div>
            ...
            <button onClick={() => includeProvider((current) => !current)}>{isProviderIncluded ? 'remove provider' : 'add provider'}</button>
            <ConditionalWrap condition={isProviderIncluded} wrap={(children) => (<BasicContextDefinition.Provider value={{ value: 99 }}>
                {children}
