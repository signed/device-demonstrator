...
class SubscriptionLedger {
  ...
  addSubscriber(subscription: DefaultMediaStreamSubscription) {
    const newEntry = () => ({
      ...
      subscriptions: new Map<SubscriptionIdentifier, DefaultMediaStreamSubscription>(),
    })
  ...
  removeSubscriber(
    ...
    onNoMoreSubscribers: (stream: Promise<MediaStream>) => void = doNothing,
  ) {
...
export class RecordingDirector {
  ...
  private onCameraSelectionChangedListeners: Set<OnCameraSelectionChangedListener> =
    new Set<OnCameraSelectionChangedListener>()
  ...
  updateDevices(newDevices: Array<Device>) {
    ...
    const removedDeviceIds = this.devices
      .filter((cur) => !availableDevicesId.includes(cur.deviceId))
