...
export class MediaStreamTrackFake implements MediaStreamTrack {
    ...
    /**
     * The *`MediaStreamTrack.id`* read-only property returns a {@link DOMString} containing a unique identifier (GUID) for the track, which is generated by the user agent.
     */
    get id(): string {
    ...
    /**
     * Returns a {@link DOMString} set to `"audio"` if the track is an audio track and to `"video"`, if it is a video track.
    ...
    /**
     * Returns a {@link DOMString} containing a user agent-assigned label that identifies the track source, as in `"internal microphone"`.
