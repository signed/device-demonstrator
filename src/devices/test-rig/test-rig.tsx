...
export const TestRig: React.FC<{}> = () => {
  ...
  useEffect(() => {
    ...
    const scenarioConstraintsAsString =
      scenarioConstrains === undefined ? 'undefined' : JSON.stringify(scenarioConstrains, null, 2)
    ...
    <div>
      ...
      <div style={banana}>
        <div style={blub}>
          ...
          <select name="scenarios" value={selectedScenario} onChange={(e) => setSelectedScenario(e.target.value)}>
            {Array.from(scenarios.all.keys()).map((summary) => (
        ...
        <button disabled={getUserMediaResult === null} onClick={handleRunChecks}>
          run checks
        </button>
        <button disabled={results.length === 0} onClick={handleClearChecks}>
          clear checks
        </button>
        ...
        <ul key={'results'}>
          {results.map((result, checkIndex) => {
            ...
            const messagesView = showMessages ? (
              <ul key={`message ${checkIndex}`}>
                {messages.map((message, messageIndex) => (
                  <li key={`message ${checkIndex} ${messageIndex}`}>{message}</li>
                ))}
              </ul>
            ) : null

            return (
              <React.Fragment key={`doombuggy ${checkIndex}`}>
      ...
      {getUserMediaResult === null ? null : getUserMediaResult instanceof MediaStream ? (
